...
class Canvas {
  ...
  constructor(props: Props) {
    ...
    this._zoomBlurFilter = new ZoomBlurFilter({
      strength: defaultSpeed / 10,
      center: [this._size / 2, this._size / 2],
    });
    this._glitchFilter = glitch
      ? new GlitchFilter({ fillMode: 4, direction: 90 })
      : null;
    this._cameraZ = 0;
    this._fov = fov;
  ...
  private _renderStars() {
    ...
    this._stars.forEach((star) => {
      ...
      starSprite.y = star.y * (this._fov / z) * screen.width +
        screen.height / 2;
